...
public class UserService : IUserService
{
    ...
    public async Task<ResponseDto<List<UserListDto>>> GetAllAsync()
    {
        ...
        var dtos = users.Select(u => new UserListDto
        {
            ...
            Role = u.UserRoles.FirstOrDefault()?.Role.RoleName ?? "No Role",
            IsActive = u.IsActive,
    ...
    public async Task<ResponseDto<UserListDto>> GetByIdAsync(Guid userId)
    {
        ...
        if (u == null) return ResponseDto<UserListDto>.FailureResult("User not found");
    ...
    public async Task<ResponseDto<bool>> UpdateStaffAsync(Guid userId, UpdateStaffDto dto)
    {
        ...
        if (user == null) return ResponseDto<bool>.FailureResult("User not found");
        ...
        return ResponseDto<bool>.SuccessResult(true, "Staff updated successful");
    }
    ...
    public async Task<ResponseDto<Guid>> CreateStaffAsync(CreateStaffDto dto)
    {
        ...
        if (existing) return ResponseDto<Guid>.FailureResult("Username already exists");
        ...
        return ResponseDto<Guid>.SuccessResult(user.Id, "Staff created");
    }
    ...
    public async Task<ResponseDto<bool>> SetUserRolesAsync(Guid userId, List<int> roleIds)
    {
        ...
        await _unitOfWork.SaveChangesAsync();
        return ResponseDto<bool>.SuccessResult(true, "Roles updated");
    }
}
